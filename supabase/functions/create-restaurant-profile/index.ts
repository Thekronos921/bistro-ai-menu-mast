...
serve(async (req) => {
  ...
  try {
    ...
    // First, create the restaurant (without owner initially)
    const { data: restaurantResult, error: restaurantError } = await supabaseServiceClient
      ...
      .insert({
        ...
        owner_user_id: null // Initially null to avoid foreign key issues
      })
      .select()
      .single()
    ...
    // Now create the user profile with the restaurant_id
    ...
    if (userError) {
      ...
      // Clean up restaurant if user creation fails
    ...
    // Finally, update the restaurant with the correct owner_user_id
    ...
    if (updateError) {
      console.error('Error updating restaurant with owner:', updateError)
