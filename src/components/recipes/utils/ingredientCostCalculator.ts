...
export const calculateTotalCost = (recipeIngredients: LocalRecipeIngredient[]) => {
  ...
  return recipeIngredients.reduce((total, recipeIngredient) => {
    ...
    // Se è un semilavorato, gestisci la conversione delle unità
    if (recipeIngredient.is_semilavorato) {
      ...
      if (recipeUnit !== baseUnit && areUnitsCompatible(recipeUnit, baseUnit)) {
        try {
          ...
          console.log(`Calcolo costo semilavorato per ${recipeIngredient.ingredient.name}:`);
          ...
          console.log(`- Costo per ${baseUnit}: €${baseCost}`);
          console.log(`- Costo parziale: €${(baseCost * quantityInBaseUnit).toFixed(2)}`);
          
          return total + (baseCost * quantityInBaseUnit);
        } catch (error) {
          console.error("Errore nella conversione per semilavorato:", error);
